import xml2js from "xml2js";
import fs from "fs";

function parseCreditReport(xmlFile) {
  const xmlString = fs.readFileSync(xmlFile, "utf-8");
  const parser = new xml2js.Parser({ explicitArray: true });

  return new Promise((resolve, reject) => {
    parser.parseString(xmlString, (err, result) => {
      if (err) {
        return reject(new Error("Failed to parse XML: " + err.message));
      }

      try {
        const response = result.INProfileResponse;

        // --- Path Aliases for easier access ---
        const applicantDetails = response?.Current_Application?.[0]?.Current_Application_Details?.[0]?.Current_Applicant_Details?.[0];
        const caisSummary = response?.CAIS_Account?.[0]?.CAIS_Summary?.[0];
        const creditAccountSummary = caisSummary?.Credit_Account?.[0];
        const outstandingBalance = caisSummary?.Total_Outstanding_Balance?.[0];
        const capsSummary = response?.TotalCAPS_Summary?.[0];
        const score = response?.SCORE?.[0];
        const accountDetailsList = response?.CAIS_Account?.[0]?.CAIS_Account_DETAILS || [];

        // --- Basic Details Extraction ---
        const firstName = applicantDetails?.First_Name?.[0] || '';
        const lastName = applicantDetails?.Last_Name?.[0] || '';
        const pan = accountDetailsList
          .map(acc => acc?.CAIS_Holder_Details?.[0]?.Income_TAX_PAN?.[0])
          .find(p => p) || 'N/A'; // Find the first available PAN in account details

        const basicDetails = {
          name: `${firstName} ${lastName}`.trim(),
          mobilePhone: applicantDetails?.MobilePhoneNumber?.[0] || 'N/A',
          pan: pan,
          creditScore: parseInt(score?.BureauScore?.[0], 10) || null,
        };

        // --- Report Summary Extraction ---
        const reportSummary = {
          totalAccounts: parseInt(creditAccountSummary?.CreditAccountTotal?.[0], 10) || 0,
          activeAccounts: parseInt(creditAccountSummary?.CreditAccountActive?.[0], 10) || 0,
          closedAccounts: parseInt(creditAccountSummary?.CreditAccountClosed?.[0], 10) || 0,
          currentBalanceAmount: parseFloat(outstandingBalance?.Outstanding_Balance_All?.[0]) || 0,
          securedAccountsAmount: parseFloat(outstandingBalance?.Outstanding_Balance_Secured?.[0]) || 0,
          unsecuredAccountsAmount: parseFloat(outstandingBalance?.Outstanding_Balance_UnSecured?.[0]) || 0,
          last7DaysCreditEnquiries: parseInt(capsSummary?.TotalCAPSLast7Days?.[0], 10) || 0,
        };

        // --- Credit Account Information Extraction ---
        const creditAccounts = accountDetailsList.map(account => {
          const addressDetails = account?.CAIS_Holder_Address_Details?.[0];
          const fullAddress = addressDetails ?
            [
              addressDetails.First_Line_Of_Address_non_normalized?.[0],
              addressDetails.Second_Line_Of_Address_non_normalized?.[0],
              addressDetails.Third_Line_Of_Address_non_normalized?.[0],
              addressDetails.City_non_normalized?.[0],
              addressDetails.State_non_normalized?.[0],
              addressDetails.ZIP_Postal_Code_non_normalized?.[0]
            ].filter(Boolean).join(', ').trim()
            : 'N/A';

          return {
            accountNumber: account?.Account_Number?.[0] || 'N/A',
            bank: account?.Subscriber_Name?.[0]?.trim() || 'N/A',
            isCreditCard: account?.Account_Type?.[0] === '10', // Assuming Account_Type '10' is Credit Card
            currentBalance: parseFloat(account?.Current_Balance?.[0]) || 0,
            amountOverdue: parseFloat(account?.Amount_Past_Due?.[0]) || 0,
            address: fullAddress,
          };
        });

        resolve({
          basicDetails,
          reportSummary,
          creditAccounts,
        });
      } catch (e) {
        reject(new Error("Error processing parsed XML data: " + e.message));
      } finally {
        fs.unlinkSync(xmlFile);
      }
    });
  });
}

// --- Example ---
const xmlData = `
<INProfileResponse>
    <Header>
        <SystemCode>0</SystemCode>
        <MessageText/>
        <ReportDate>20200723</ReportDate>
        <ReportTime>171559</ReportTime>
    </Header>
    <UserMessage>
        <UserMessageText>Normal Response</UserMessageText>
    </UserMessage>
    <CreditProfileHeader>
        <Enquiry_Username>batchusr_matchtest_noinquiry_ecv_custom_match</Enquiry_Username>
        <ReportDate>20200723</ReportDate>
        <ReportTime>171559</ReportTime>
        <Version>V2.4</Version>
        <ReportNumber>1595504758919</ReportNumber>
        <Subscriber/>
        <Subscriber_Name>Bureau Disclosure Report with Customized Match</Subscriber_Name>
    </CreditProfileHeader>
    <Current_Application>
        <Current_Application_Details>
            <Enquiry_Reason>6</Enquiry_Reason>
            <Finance_Purpose/>
            <Amount_Financed>0</Amount_Financed>
            <Duration_Of_Agreement>0</Duration_Of_Agreement>
            <Current_Applicant_Details>
                <Last_Name>ugle</Last_Name>
                <First_Name>Sagar</First_Name>
                <Middle_Name1/>
                <Middle_Name2/>
                <Middle_Name3/>
                <Gender_Code/>
                <IncomeTaxPan/>
                <PAN_Issue_Date/>
                <PAN_Expiration_Date/>
                <Passport_number/>
                <Passport_Issue_Date/>
                <Passport_Expiration_Date/>
                <Voter_s_Identity_Card/>
                <Voter_ID_Issue_Date/>
                <Voter_ID_Expiration_Date/>
                <Driver_License_Number/>
                <Driver_License_Issue_Date/>
                <Driver_License_Expiration_Date/>
                <Ration_Card_Number/>
                <Ration_Card_Issue_Date/>
                <Ration_Card_Expiration_Date/>
                <Universal_ID_Number/>
                <Universal_ID_Issue_Date/>
                <Universal_ID_Expiration_Date/>
                <Date_Of_Birth_Applicant>00010201</Date_Of_Birth_Applicant>
                <Telephone_Number_Applicant_1st/>
                <Telephone_Extension/>
                <Telephone_Type/>
                <MobilePhoneNumber>9819137672</MobilePhoneNumber>
                <EMailId/>
            </Current_Applicant_Details>
            <Current_Other_Details>
                <Income>0</Income>
                <Marital_Status/>
                <Employment_Status/>
                <Time_with_Employer/>
                <Number_of_Major_Credit_Card_Held/>
            </Current_Other_Details>
            <Current_Applicant_Address_Details>
                <FlatNoPlotNoHouseNo/>
                <BldgNoSocietyName/>
                <RoadNoNameAreaLocality/>
                <City/>
                <Landmark/>
                <State/>
                <PINCode/>
                <Country_Code>IB</Country_Code>
            </Current_Applicant_Address_Details>
            <Current_Applicant_Additional_AddressDetails/>
        </Current_Application_Details>
    </Current_Application>
    <CAIS_Account>
        <CAIS_Summary>
            <Credit_Account>
                <CreditAccountTotal>4</CreditAccountTotal>
                <CreditAccountActive>3</CreditAccountActive>
                <CreditAccountDefault>0</CreditAccountDefault>
                <CreditAccountClosed>1</CreditAccountClosed>
                <CADSuitFiledCurrentBalance>0</CADSuitFiledCurrentBalance>
            </Credit_Account>
            <Total_Outstanding_Balance>
                <Outstanding_Balance_Secured>85000</Outstanding_Balance_Secured>
                <Outstanding_Balance_Secured_Percentage>35</Outstanding_Balance_Secured_Percentage>
                <Outstanding_Balance_UnSecured>160000</Outstanding_Balance_UnSecured>
                <Outstanding_Balance_UnSecured_Percentage>65</Outstanding_Balance_UnSecured_Percentage>
                <Outstanding_Balance_All>245000</Outstanding_Balance_All>
            </Total_Outstanding_Balance>
        </CAIS_Summary>
        <CAIS_Account_DETAILS>
            <Identification_Number/>
            <Subscriber_Name> icicibank</Subscriber_Name>
            <Account_Number>ICIVB20994</Account_Number>
            <Portfolio_Type>R</Portfolio_Type>
            <Account_Type>10</Account_Type>
            <Open_Date>19981213</Open_Date>
            <Credit_Limit_Amount>50000</Credit_Limit_Amount>
            <Highest_Credit_or_Original_Loan_Amount/>
            <Terms_Duration/>
            <Terms_Frequency/>
            <Scheduled_Monthly_Payment_Amount/>
            <Account_Status>11</Account_Status>
            <Payment_Rating>0</Payment_Rating>
            <Payment_History_Profile>N</Payment_History_Profile>
            <Special_Comment/>
            <Current_Balance>80000</Current_Balance>
            <Amount_Past_Due>4000</Amount_Past_Due>
            <Original_Charge_Off_Amount/>
            <Date_Reported>20180920</Date_Reported>
            <Date_of_First_Delinquency/>
            <Date_Closed/>
            <Date_of_Last_Payment/>
            <SuitFiledWillfulDefaultWrittenOffStatus/>
            <SuitFiled_WilfulDefault/>
            <Written_off_Settled_Status/>
            <Value_of_Credits_Last_Month/>
            <Occupation_Code/>
            <Settlement_Amount/>
            <Value_of_Collateral/>
            <Type_of_Collateral/>
            <Written_Off_Amt_Total/>
            <Written_Off_Amt_Principal/>
            <Rate_of_Interest/>
            <Repayment_Tenure>0</Repayment_Tenure>
            <Promotional_Rate_Flag/>
            <Income/>
            <Income_Indicator/>
            <Income_Frequency_Indicator/>
            <DefaultStatusDate/>
            <LitigationStatusDate/>
            <WriteOffStatusDate/>
            <DateOfAddition>20180920</DateOfAddition>
            <CurrencyCode>INR</CurrencyCode>
            <Subscriber_comments/>
            <Consumer_comments/>
            <AccountHoldertypeCode>1</AccountHoldertypeCode>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>09</Month>
                <Days_Past_Due>20</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Holder_Details>
                <Surname_Non_Normalized>SAGAR</Surname_Non_Normalized>
                <First_Name_Non_Normalized>UGLE</First_Name_Non_Normalized>
                <Middle_Name_1_Non_Normalized/>
                <Middle_Name_2_Non_Normalized/>
                <Middle_Name_3_Non_Normalized/>
                <Alias/>
                <Gender_Code>1</Gender_Code>
                <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
                <Passport_Number/>
                <Voter_ID_Number/>
                <Date_of_birth>19820322</Date_of_birth>
            </CAIS_Holder_Details>
            <CAIS_Holder_Address_Details>
                <First_Line_Of_Address_non_normalized>ANANDI VIHAR</First_Line_Of_Address_non_normalized>
                <Second_Line_Of_Address_non_normalized>DEHU ROAD</Second_Line_Of_Address_non_normalized>
                <Third_Line_Of_Address_non_normalized/>
                <City_non_normalized>PUNE</City_non_normalized>
                <Fifth_Line_Of_Address_non_normalized/>
                <State_non_normalized>27</State_non_normalized>
                <ZIP_Postal_Code_non_normalized>411047</ZIP_Postal_Code_non_normalized>
                <CountryCode_non_normalized>IB</CountryCode_non_normalized>
                <Address_indicator_non_normalized/>
                <Residence_code_non_normalized/>
            </CAIS_Holder_Address_Details>
            <CAIS_Holder_Phone_Details>
                <Telephone_Number>9819137672</Telephone_Number>
                <Telephone_Type/>
                <Telephone_Extension/>
                <Mobile_Telephone_Number/>
                <FaxNumber/>
                <EMailId/>
            </CAIS_Holder_Phone_Details>
            <CAIS_Holder_ID_Details>
                <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
                <PAN_Issue_Date/>
                <PAN_Expiration_Date/>
                <Passport_Number/>
                <Passport_Issue_Date/>
                <Passport_Expiration_Date/>
                <Voter_ID_Number/>
                <Voter_ID_Issue_Date/>
                <Voter_ID_Expiration_Date/>
                <Driver_License_Number/>
                <Driver_License_Issue_Date/>
                <Driver_License_Expiration_Date/>
                <Ration_Card_Number/>
                <Ration_Card_Issue_Date/>
                <Ration_Card_Expiration_Date/>
                <Universal_ID_Number/>
                <Universal_ID_Issue_Date/>
                <Universal_ID_Expiration_Date/>
                <EMailId/>
            </CAIS_Holder_ID_Details>
            <CAIS_Holder_ID_Details>
                <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
                <PAN_Issue_Date/>
                <PAN_Expiration_Date/>
                <Passport_Number/>
                <Passport_Issue_Date/>
                <Passport_Expiration_Date/>
                <Voter_ID_Number/>
                <Voter_ID_Issue_Date/>
                <Voter_ID_Expiration_Date/>
                <Driver_License_Number/>
                <Driver_License_Issue_Date/>
                <Driver_License_Expiration_Date/>
                <Ration_Card_Number/>
                <Ration_Card_Issue_Date/>
                <Ration_Card_Expiration_Date/>
                <Universal_ID_Number/>
                <Universal_ID_Issue_Date/>
                <Universal_ID_Expiration_Date/>
                <EMailId/>
            </CAIS_Holder_ID_Details>
        </CAIS_Account_DETAILS>
        <CAIS_Account_DETAILS>
            <Identification_Number/>
            <Subscriber_Name> icicibank</Subscriber_Name>
            <Account_Number>ICICI8131308</Account_Number>
            <Portfolio_Type>I</Portfolio_Type>
            <Account_Type>52</Account_Type>
            <Open_Date>19940718</Open_Date>
            <Credit_Limit_Amount/>
            <Highest_Credit_or_Original_Loan_Amount>50000</Highest_Credit_or_Original_Loan_Amount>
            <Terms_Duration/>
            <Terms_Frequency/>
            <Scheduled_Monthly_Payment_Amount/>
            <Account_Status>13</Account_Status>
            <Payment_Rating>0</Payment_Rating>
            <Payment_History_Profile>11110000000000001???????????????????</Payment_History_Profile>
            <Special_Comment/>
            <Current_Balance>0</Current_Balance>
            <Amount_Past_Due>0</Amount_Past_Due>
            <Original_Charge_Off_Amount/>
            <Date_Reported>20190130</Date_Reported>
            <Date_of_First_Delinquency/>
            <Date_Closed>20190110</Date_Closed>
            <Date_of_Last_Payment/>
            <SuitFiledWillfulDefaultWrittenOffStatus/>
            <SuitFiled_WilfulDefault/>
            <Written_off_Settled_Status/>
            <Value_of_Credits_Last_Month/>
            <Occupation_Code/>
            <Settlement_Amount/>
            <Value_of_Collateral/>
            <Type_of_Collateral/>
            <Written_Off_Amt_Total/>
            <Written_Off_Amt_Principal/>
            <Rate_of_Interest/>
            <Repayment_Tenure>0</Repayment_Tenure>
            <Promotional_Rate_Flag/>
            <Income/>
            <Income_Indicator/>
            <Income_Frequency_Indicator/>
            <DefaultStatusDate/>
            <LitigationStatusDate/>
            <WriteOffStatusDate/>
            <DateOfAddition>20170830</DateOfAddition>
            <CurrencyCode>INR</CurrencyCode>
            <Subscriber_comments/>
            <Consumer_comments/>
            <AccountHoldertypeCode>1</AccountHoldertypeCode>
            <CAIS_Account_History>
                <Year>2019</Year>
                <Month>01</Month>
                <Days_Past_Due>0</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>12</Month>
                <Days_Past_Due>40</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>11</Month>
                <Days_Past_Due>38</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>10</Month>
                <Days_Past_Due>36</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>09</Month>
                <Days_Past_Due>34</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>08</Month>
                <Days_Past_Due>25</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>07</Month>
                <Days_Past_Due>24</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>06</Month>
                <Days_Past_Due>23</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>05</Month>
                <Days_Past_Due>22</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>04</Month>
                <Days_Past_Due>21</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>03</Month>
                <Days_Past_Due>20</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>02</Month>
                <Days_Past_Due>19</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>01</Month>
                <Days_Past_Due>18</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>12</Month>
                <Days_Past_Due>17</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>11</Month>
                <Days_Past_Due>16</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>10</Month>
                <Days_Past_Due>15</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>09</Month>
                <Days_Past_Due>14</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>08</Month>
                <Days_Past_Due>45</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Holder_Details>
                <Surname_Non_Normalized>SAGAR</Surname_Non_Normalized>
                <First_Name_Non_Normalized>UGLE</First_Name_Non_Normalized>
                <Middle_Name_1_Non_Normalized/>
                <Middle_Name_2_Non_Normalized/>
                <Middle_Name_3_Non_Normalized/>
                <Alias/>
                <Gender_Code>1</Gender_Code>
                <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
                <Passport_Number/>
                <Voter_ID_Number/>
                <Date_of_birth>19820322</Date_of_birth>
            </CAIS_Holder_Details>
            <CAIS_Holder_Address_Details>
                <First_Line_Of_Address_non_normalized>J-1 110</First_Line_Of_Address_non_normalized>
                <Second_Line_Of_Address_non_normalized>ANANDI VIHAR</Second_Line_Of_Address_non_normalized>
                <Third_Line_Of_Address_non_normalized>DEHU ROAD</Third_Line_Of_Address_non_normalized>
                <City_non_normalized>DIGHI</City_non_normalized>
                <Fifth_Line_Of_Address_non_normalized/>
                <State_non_normalized>27</State_non_normalized>
                <ZIP_Postal_Code_non_normalized>412216</ZIP_Postal_Code_non_normalized>
                <CountryCode_non_normalized>IB</CountryCode_non_normalized>
                <Address_indicator_non_normalized/>
                <Residence_code_non_normalized/>
            </CAIS_Holder_Address_Details>
            <CAIS_Holder_Phone_Details>
                <Telephone_Number>9819137672</Telephone_Number>
                <Telephone_Type/>
                <Telephone_Extension/>
                <Mobile_Telephone_Number/>
                <FaxNumber/>
                <EMailId/>
            </CAIS_Holder_Phone_Details>
            <CAIS_Holder_ID_Details>
                <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
                <PAN_Issue_Date/>
                <PAN_Expiration_Date/>
                <Passport_Number/>
                <Passport_Issue_Date/>
                <Passport_Expiration_Date/>
                <Voter_ID_Number/>
                <Voter_ID_Issue_Date/>
                <Voter_ID_Expiration_Date/>
                <Driver_License_Number/>
                <Driver_License_Issue_Date/>
                <Driver_License_Expiration_Date/>
                <Ration_Card_Number/>
                <Ration_Card_Issue_Date/>
                <Ration_Card_Expiration_Date/>
                <Universal_ID_Number/>
                <Universal_ID_Issue_Date/>
                <Universal_ID_Expiration_Date/>
                <EMailId/>
            </CAIS_Holder_ID_Details>
        </CAIS_Account_DETAILS>
        <CAIS_Account_DETAILS>
            <Identification_Number/>
            <Subscriber_Name> icicibank</Subscriber_Name>
            <Account_Number>ICICI8131306</Account_Number>
            <Portfolio_Type>R</Portfolio_Type>
            <Account_Type>10</Account_Type>
            <Open_Date>19940716</Open_Date>
            <Credit_Limit_Amount>50000</Credit_Limit_Amount>
            <Highest_Credit_or_Original_Loan_Amount>50000</Highest_Credit_or_Original_Loan_Amount>
            <Terms_Duration/>
            <Terms_Frequency/>
            <Scheduled_Monthly_Payment_Amount/>
            <Account_Status>53</Account_Status>
            <Payment_Rating>1</Payment_Rating>
            <Payment_History_Profile>11110000000000001???????????????????</Payment_History_Profile>
            <Special_Comment/>
            <Current_Balance>80000</Current_Balance>
            <Amount_Past_Due>1100</Amount_Past_Due>
            <Original_Charge_Off_Amount/>
            <Date_Reported>20190130</Date_Reported>
            <Date_of_First_Delinquency/>
            <Date_Closed/>
            <Date_of_Last_Payment/>
            <SuitFiledWillfulDefaultWrittenOffStatus/>
            <SuitFiled_WilfulDefault>01</SuitFiled_WilfulDefault>
            <Written_off_Settled_Status>00</Written_off_Settled_Status>
            <Value_of_Credits_Last_Month/>
            <Occupation_Code/>
            <Settlement_Amount/>
            <Value_of_Collateral/>
            <Type_of_Collateral/>
            <Written_Off_Amt_Total/>
            <Written_Off_Amt_Principal/>
            <Rate_of_Interest/>
            <Repayment_Tenure>0</Repayment_Tenure>
            <Promotional_Rate_Flag/>
            <Income/>
            <Income_Indicator/>
            <Income_Frequency_Indicator/>
            <DefaultStatusDate/>
            <LitigationStatusDate/>
            <WriteOffStatusDate/>
            <DateOfAddition>20170830</DateOfAddition>
            <CurrencyCode>INR</CurrencyCode>
            <Subscriber_comments/>
            <Consumer_comments/>
            <AccountHoldertypeCode>1</AccountHoldertypeCode>
            <CAIS_Account_History>
                <Year>2019</Year>
                <Month>01</Month>
                <Days_Past_Due>36</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>12</Month>
                <Days_Past_Due>36</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>11</Month>
                <Days_Past_Due>34</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>10</Month>
                <Days_Past_Due>32</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>09</Month>
                <Days_Past_Due>30</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>08</Month>
                <Days_Past_Due>23</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>07</Month>
                <Days_Past_Due>22</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>06</Month>
                <Days_Past_Due>21</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>05</Month>
                <Days_Past_Due>20</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>04</Month>
                <Days_Past_Due>19</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>03</Month>
                <Days_Past_Due>18</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>02</Month>
                <Days_Past_Due>17</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>01</Month>
                <Days_Past_Due>16</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>12</Month>
                <Days_Past_Due>15</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>11</Month>
                <Days_Past_Due>14</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>10</Month>
                <Days_Past_Due>13</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>09</Month>
                <Days_Past_Due>12</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>08</Month>
                <Days_Past_Due>35</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Holder_Details>
                <Surname_Non_Normalized>SAGAR</Surname_Non_Normalized>
                <First_Name_Non_Normalized>UGLE</First_Name_Non_Normalized>
                <Middle_Name_1_Non_Normalized/>
                <Middle_Name_2_Non_Normalized/>
                <Middle_Name_3_Non_Normalized/>
                <Alias/>
                <Gender_Code>1</Gender_Code>
                <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
                <Passport_Number/>
                <Voter_ID_Number/>
                <Date_of_birth>19820322</Date_of_birth>
            </CAIS_Holder_Details>
            <CAIS_Holder_Address_Details>
                <First_Line_Of_Address_non_normalized>J-1 110</First_Line_Of_Address_non_normalized>
                <Second_Line_Of_Address_non_normalized>ANANDI VIHAR</Second_Line_Of_Address_non_normalized>
                <Third_Line_Of_Address_non_normalized>DEHU ROAD</Third_Line_Of_Address_non_normalized>
                <City_non_normalized>DIGHI</City_non_normalized>
                <Fifth_Line_Of_Address_non_normalized/>
                <State_non_normalized>27</State_non_normalized>
                <ZIP_Postal_Code_non_normalized>412216</ZIP_Postal_Code_non_normalized>
                <CountryCode_non_normalized>IB</CountryCode_non_normalized>
                <Address_indicator_non_normalized/>
                <Residence_code_non_normalized/>
            </CAIS_Holder_Address_Details>
            <CAIS_Holder_Phone_Details>
                <Telephone_Number>9819137672</Telephone_Number>
                <Telephone_Type/>
                <Telephone_Extension/>
                <Mobile_Telephone_Number/>
                <FaxNumber/>
                <EMailId/>
            </CAIS_Holder_Phone_Details>
            <CAIS_Holder_ID_Details>
                <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
                <PAN_Issue_Date/>
                <PAN_Expiration_Date/>
                <Passport_Number/>
                <Passport_Issue_Date/>
                <Passport_Expiration_Date/>
                <Voter_ID_Number/>
                <Voter_ID_Issue_Date/>
                <Voter_ID_Expiration_Date/>
                <Driver_License_Number/>
                <Driver_License_Issue_Date/>
                <Driver_License_Expiration_Date/>
                <Ration_Card_Number/>
                <Ration_Card_Issue_Date/>
                <Ration_Card_Expiration_Date/>
                <Universal_ID_Number/>
                <Universal_ID_Issue_Date/>
                <Universal_ID_Expiration_Date/>
                <EMailId/>
            </CAIS_Holder_ID_Details>
        </CAIS_Account_DETAILS>
        <CAIS_Account_DETAILS>
            <Identification_Number/>
            <Subscriber_Name> icicibank</Subscriber_Name>
            <Account_Number>ICICI8131307</Account_Number>
            <Portfolio_Type>I</Portfolio_Type>
            <Account_Type>51</Account_Type>
            <Open_Date>19940717</Open_Date>
            <Credit_Limit_Amount/>
            <Highest_Credit_or_Original_Loan_Amount>50000</Highest_Credit_or_Original_Loan_Amount>
            <Terms_Duration/>
            <Terms_Frequency/>
            <Scheduled_Monthly_Payment_Amount/>
            <Account_Status>71</Account_Status>
            <Payment_Rating>1</Payment_Rating>
            <Payment_History_Profile>11110000000000001???????????????????</Payment_History_Profile>
            <Special_Comment/>
            <Current_Balance>85000</Current_Balance>
            <Amount_Past_Due>1200</Amount_Past_Due>
            <Original_Charge_Off_Amount/>
            <Date_Reported>20190130</Date_Reported>
            <Date_of_First_Delinquency/>
            <Date_Closed/>
            <Date_of_Last_Payment/>
            <SuitFiledWillfulDefaultWrittenOffStatus/>
            <SuitFiled_WilfulDefault/>
            <Written_off_Settled_Status/>
            <Value_of_Credits_Last_Month/>
            <Occupation_Code/>
            <Settlement_Amount/>
            <Value_of_Collateral/>
            <Type_of_Collateral/>
            <Written_Off_Amt_Total/>
            <Written_Off_Amt_Principal/>
            <Rate_of_Interest/>
            <Repayment_Tenure>0</Repayment_Tenure>
            <Promotional_Rate_Flag/>
            <Income/>
            <Income_Indicator/>
            <Income_Frequency_Indicator/>
            <DefaultStatusDate/>
            <LitigationStatusDate/>
            <WriteOffStatusDate/>
            <DateOfAddition>20170830</DateOfAddition>
            <CurrencyCode>INR</CurrencyCode>
            <Subscriber_comments/>
            <Consumer_comments/>
            <AccountHoldertypeCode>1</AccountHoldertypeCode>
            <CAIS_Account_History>
                <Year>2019</Year>
                <Month>01</Month>
                <Days_Past_Due>38</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>12</Month>
                <Days_Past_Due>38</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>11</Month>
                <Days_Past_Due>36</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>10</Month>
                <Days_Past_Due>34</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>09</Month>
                <Days_Past_Due>32</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>08</Month>
                <Days_Past_Due>24</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>07</Month>
                <Days_Past_Due>23</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>06</Month>
                <Days_Past_Due>22</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>05</Month>
                <Days_Past_Due>21</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>04</Month>
                <Days_Past_Due>20</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>03</Month>
                <Days_Past_Due>19</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>02</Month>
                <Days_Past_Due>18</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2018</Year>
                <Month>01</Month>
                <Days_Past_Due>17</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>12</Month>
                <Days_Past_Due>16</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>11</Month>
                <Days_Past_Due>15</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>10</Month>
                <Days_Past_Due>14</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>09</Month>
                <Days_Past_Due>13</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Account_History>
                <Year>2017</Year>
                <Month>08</Month>
                <Days_Past_Due>40</Days_Past_Due>
                <Asset_Classification>?</Asset_Classification>
            </CAIS_Account_History>
            <CAIS_Holder_Details>
                <Surname_Non_Normalized>SAGAR</Surname_Non_Normalized>
                <First_Name_Non_Normalized>UGLE</First_Name_Non_Normalized>
                <Middle_Name_1_Non_Normalized/>
                <Middle_Name_2_Non_Normalized/>
                <Middle_Name_3_Non_Normalized/>
                <Alias/>
                <Gender_Code>1</Gender_Code>
                <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
                <Passport_Number/>
                <Voter_ID_Number/>
                <Date_of_birth>19820322</Date_of_birth>
            </CAIS_Holder_Details>
            <CAIS_Holder_Address_Details>
                <First_Line_Of_Address_non_normalized>J-1 110</First_Line_Of_Address_non_normalized>
                <Second_Line_Of_Address_non_normalized>ANANDI VIHAR</Second_Line_Of_Address_non_normalized>
                <Third_Line_Of_Address_non_normalized>DEHU ROAD</Third_Line_Of_Address_non_normalized>
                <City_non_normalized>DIGHI</City_non_normalized>
                <Fifth_Line_Of_Address_non_normalized/>
                <State_non_normalized>27</State_non_normalized>
                <ZIP_Postal_Code_non_normalized>412216</ZIP_Postal_Code_non_normalized>
                <CountryCode_non_normalized>IB</CountryCode_non_normalized>
                <Address_indicator_non_normalized/>
                <Residence_code_non_normalized/>
            </CAIS_Holder_Address_Details>
            <CAIS_Holder_Phone_Details>
                <Telephone_Number>9819137672</Telephone_Number>
                <Telephone_Type/>
                <Telephone_Extension/>
                <Mobile_Telephone_Number/>
                <FaxNumber/>
                <EMailId/>
            </CAIS_Holder_Phone_Details>
            <CAIS_Holder_ID_Details>
                <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
                <PAN_Issue_Date/>
                <PAN_Expiration_Date/>
                <Passport_Number/>
                <Passport_Issue_Date/>
                <Passport_Expiration_Date/>
                <Voter_ID_Number/>
                <Voter_ID_Issue_Date/>
                <Voter_ID_Expiration_Date/>
                <Driver_License_Number/>
                <Driver_License_Issue_Date/>
                <Driver_License_Expiration_Date/>
                <Ration_Card_Number/>
                <Ration_Card_Issue_Date/>
                <Ration_Card_Expiration_Date/>
                <Universal_ID_Number/>
                <Universal_ID_Issue_Date/>
                <Universal_ID_Expiration_Date/>
                <EMailId/>
            </CAIS_Holder_ID_Details>
        </CAIS_Account_DETAILS>
    </CAIS_Account>
    <Match_result>
        <Exact_match>Y</Exact_match>
    </Match_result>
    <TotalCAPS_Summary>
        <TotalCAPSLast7Days>0</TotalCAPSLast7Days>
        <TotalCAPSLast30Days>0</TotalCAPSLast30Days>
        <TotalCAPSLast90Days>0</TotalCAPSLast90Days>
        <TotalCAPSLast180Days>0</TotalCAPSLast180Days>
    </TotalCAPS_Summary>
    <CAPS>
        <CAPS_Summary>
            <CAPSLast7Days>0</CAPSLast7Days>
            <CAPSLast30Days>0</CAPSLast30Days>
            <CAPSLast90Days>0</CAPSLast90Days>
            <CAPSLast180Days>0</CAPSLast180Days>
        </CAPS_Summary>
    </CAPS>
    <NonCreditCAPS>
        <NonCreditCAPS_Summary>
            <NonCreditCAPSLast7Days>0</NonCreditCAPSLast7Days>
            <NonCreditCAPSLast30Days>0</NonCreditCAPSLast30Days>
            <NonCreditCAPSLast90Days>0</NonCreditCAPSLast90Days>
            <NonCreditCAPSLast180Days>0</NonCreditCAPSLast180Days>
        </NonCreditCAPS_Summary>
    </NonCreditCAPS>
    <SCORE>
        <BureauScore>719</BureauScore>
        <BureauScoreConfidLevel>H</BureauScoreConfidLevel>
    </SCORE>
</INProfileResponse>
`;


// (async () => {
//   try {
//     const jsonData = await parseCreditReport(xmlData);
//     console.log(JSON.stringify(jsonData, null, 2));
//   } catch (error) {
//     console.error(error);
//   }
// })();

export default parseCreditReport;